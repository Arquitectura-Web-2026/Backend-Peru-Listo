...
import com.upc.presulisto.DTO.PasswordResetRequest;
...
public class UsuarioService {
    ...
    public ResponseEntity<?> cambiarPassword(Long id, PasswordChangeRequest request) {
        ...
        if (!passwordEncoder.matches(request.getCurrentPassword(), usuario.getPasswordHash())) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("Error: La contraseña actual es incorrecta"));
        }

        if (request.getNewPassword().length() < 8) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("Error: La nueva contraseña debe tener al menos 8 caracteres"));
        }

        if (passwordEncoder.matches(request.getNewPassword(), usuario.getPasswordHash())) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("Error: La nueva contraseña debe ser diferente a la actual"));
        }

        if (!request.getNewPassword().equals(request.getConfirmPassword())) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("Error: Las contraseñas no coinciden"));
        }

        usuario.setPasswordHash(passwordEncoder.encode(request.getNewPassword()));
        usuarioRepository.save(usuario);

        return ResponseEntity.ok(new com.upc.presulisto.DTO.MessageResponse("Contraseña actualizada exitosamente"));
    }
    ...
    public ResponseEntity<?> solicitarRecuperacion(String correo) {
        ...
        return ResponseEntity.ok(new com.upc.presulisto.DTO.MessageResponse(
    ...
    public ResponseEntity<?> restablecerPassword(String token, PasswordResetRequest request) {
        ...
        if (usuario.getResetTokenExpiry().isBefore(LocalDateTime.now())) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("El enlace ha expirado. Solicita uno nuevo"));
        }
        
        // Validar nueva contraseña
        if (request.getNewPassword().length() < 8) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("La contraseña debe tener al menos 8 caracteres"));
        }
        
        if (!request.getNewPassword().equals(request.getConfirmPassword())) {
            return ResponseEntity.badRequest()
                    .body(new com.upc.presulisto.DTO.MessageResponse("Las contraseñas no coinciden"));
        }
